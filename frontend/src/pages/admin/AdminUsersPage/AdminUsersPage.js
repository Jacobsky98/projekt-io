...
const AdminUsersPage = () => {
  ...
  const isFormFilled = () => {
    return username !== '' && firstName !== '' && lastName !== '' && role !== '' && password !== '' && email !== '';
  }
  ...
  return (
      <div className={'admin-user-page'}>
            ...
            {users.map((user, index) => {
              return (
                  <ListItem button key={index}>
                    ...
                    <ListItemText primary={`${user.username} ${user.email}`} secondary={`rola: ${user.role}`}/>
                  </ListItem>
              );
            })}
          </List>
        ...
        <div className={`users-form ${showUserForm ? 'users-form--active' : ''}`}>
          <TextField value={firstName} onChange={(e) => setFirstName(e.target.value)} label="Imię" /> <br />
          <TextField value={lastName} onChange={(e) => setLastName(e.target.value)} label="Nazwisko" /> <br />
          <TextField value={username} onChange={(e) => setUsername(e.target.value)} label="Username" /> <br />
          <TextField value={password} onChange={(e) => setPassword(e.target.value)} label="Hasło" /> <br />
          <TextField value={email} onChange={(e) => setEmail(e.target.value)} label="Email"/> <br />
          <FormControl>
